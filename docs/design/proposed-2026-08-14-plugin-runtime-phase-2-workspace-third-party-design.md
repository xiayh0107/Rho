...
Status: proposed security and architecture design; implementation not authorized
...
Change class: D3 safety-critical architecture. This pull request is
documentation-only. Any implementation that executes project-authored code,
...
or central cross-review matrix. Those shared records are updated only if this
proposal is accepted or one bounded implementation package is explicitly
activated. The Phase 1 and Phase 2 design pull requests therefore have no Git
base dependency and can be reviewed, revised, merged, or closed independently.
...
No tests are claimed by this documentation-only PR. An authorized Phase 2
implementation must add deterministic coverage for at least the following.
